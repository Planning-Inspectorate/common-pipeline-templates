...
$inUseDigests=@{}
foreach ($repo in $repos) {
    $inUseDigests[$repo]=@() # initialise an array per repo
}
...
foreach ($repo in $repos) {
    $repositoryManifests[$repo]=Invoke-Command -ScriptBlock {
        az acr manifest list-metadata --registry "pinscrsharedtoolinguks" --name "$repo" --output json
    }
    $repositoryManifests[$repo]=$repositoryManifests[$repo] | ConvertFrom-Json
    $repositoryManifests[$repo] | Out-String | Write-Host
}
